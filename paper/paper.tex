...
\newcommand{\pbm}[1]{\todo[inline,color=red!40]{PEF: #1}}
...
We discretize the $\Ltwo$ de Rham complex on triangular and tetrahedral meshes with finite element subcomplexes of the first and second kinds (\Cref{fig:3DL2deRham}).
...
\pef{Find this in a book somewhere and cite it}
\begin{equation}
   X^1_{p}(\Khat) = \{v\in [\P_p(K)]^d : \tr v \in \P_{p_E}(E) \Forall E \in \Delta^1(\Khat)  \}.
\end{equation}
Choosing $p_E=p-1$ we obtain the \Nedelec space of the first kind $\Ned_p(\Khat)$, while
choosing $p_E=p$ gives the \Nedelec space of the second kind $\NedTwo_p(K) = [\P_p(\Khat)]^d$.
\pef{Cite \Nedelec}
These spaces enforce continuity of the tangential trace across mesh interfaces.


We define a basis for the dual of $X^1_p(K)$ as 
tangential moments along edges $E\in \Delta^1(\Khat)$,
\begin{equation}
   \ell^E_j(v) = (q_j, v\cdot \bt)_E, \quad q_j \in \P_{p_E}(E),
\end{equation}
and for each higher-dimensional sub-entity $S \in \cup_{k=2}^d \Delta^k(\Khat)$,
\begin{align}
...
$\{\phi^S_j\}$ is the basis for $B^0_{p_S}(S)$ constructed in \eqref{eq:hgrad-fdm},
and $\{\curl_S\Phi^S_j\}$ is a basis for $\curl_S B^1_{p_S}(S)$
constructed by solving the following problem:
find $\{\Phi^S_j\} \subset B^1_{p_S}(S)$ such that
\begin{equation} \label{eq:hcurl-fdm}
   (\curl_S\Phi^S_j, \curl_S\Phi^S_i)_{S} = \delta_{ij}, \quad
   (\Phi^S_j, \Phi^S_i)_{S} = \lambda_j\delta_{ij}, \quad \lambda_j > 0.
\end{equation}
\pef{FIXME: what is $p_S$?}

The eigenbasis $\{\Phi^S_j\}$ is numerically computed offline and only once for each kind of reference subentity of dimension greater than or equal to two.
