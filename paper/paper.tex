...
We follow the definition of interpolation degrees of freedom from \cite{demkowicz00}
to construct dual bases that promote orthgonality in the $\Ltwo(\Khat)$ and $H(\mathrm{d}, \Khat)$ inner
products.
...
and $\{\phi^S_j\}$ is a basis for
$\P_{p,0}(S) = \{ v \in \P_p(S): v = 0 \text{ on } \partial S \}$, such that
...
and $\{\curl_S\Phi^S_j\}$ is a basis for $\curl_S \mathbb{X}_{p_S}(S)$,
$\mathbb{X}_{p_S}(S) = [\P_{p_S}(S)]^d \cap H_0(\curl, S)$, such that
...
We define a basis for the dual of $V^2_{p, p_F}(K)$ as 
...
\end{align}
where $\{\Phi^K_j\}$ are the $H_0(\curl, S)$ bases constructed in \eqref{eq:hcurl-fdm},
and $\{\div\Psi^K_j\}$ is a basis for $\div \mathbb{Y}_p(K)$,
$\mathbb{Y}_{p}(K) = [\mathbb{P}_p(K)]^d \cap H_0(\div,K)$, such that
\begin{equation} \label{eq:hdiv-fdm}
   (\div\Psi^K_j, \div\Psi^K_i)_{K} = \delta_{ij}, \quad
...
and extend them to the high-order with static-condensation. 
