...
\textit{De Rham diagram for $hp$ finite element spaces}. Comput.~Math.~Appl.,
39(7-8):29--38, 2000., and consist of integral moments
...
\mathrm{curl}, \mathrm{div}\}$). On the reference equilateral simplex, the
...
We discretize $\Ltwo$-de Rham complex on triangular and tetrahedral meshes. 
...
\end{equation}
On a 2D face $F$ (or the interior of a 2D cell),
it is not necessary to solve for $\{\Phi^F_j\}$, as the degrees of freedom
\eqref{eq:hcurl-interior-dofs} only involve the scalar-valued 
$\curl_F\Phi^F_j$ which spans $\P_{p_F-1}(F)\setminus\P_0(F)$,
hence the standard $\Ltwo$-orthogonal Dubiner basis may be used in place of $\curl_F\Phi^F_j$.
...
\end{equation}
It is not necessary to solve for $\{\Psi^K_j\}$, as the degrees of freedom
\eqref{eq:hdiv-interior-dofs} only involve $\div\Psi^K_j$, 
which spans $\P_{p-1}(K)\setminus\P_0(K)$. Hence the standard
$\Ltwo$-orthogonal Dubiner basis may be used in place of $\div\Psi^K_j$.


\subsection{Generally-mapped elements}

The dual bases described above yield a sparse basis for $V^k_p(K)$ only
if $K$ is equilateral.
